...
1. **页面状态不在 CLI 调用之间保持** —— 上一次 `open` 的页面，下一次调用里 snapshot 会是
   `(no interactive elements)`、screenshot 是白屏。**必须把「open → 操作 → 截图」写在
   同一个 bash 调用里**（用 `{ ...; }` 包起来顺序执行）。
   顺带一提：单独发一个 `eval`（没有同调用的 `open`）会直接**挂住不返回**，
   表现为 bash 超时 SIGTERM、没有任何输出 —— 别以为是脚本写错了。
2. **ref（`e3` 这种）在调用之间失效** —— 报 `✗ Unknown ref: e6`。
...
3. **每次调用都是全新的浏览器上下文，localStorage 不保留 → 必须每次都重新登录。**
   不带登录直接 `open /workspace`，页面外壳照常渲染（工具坞都在），但所有 `/api/*`
   都是 **401**，前端只显示一句含糊的「**AI 服务暂时不可用**」——
   很容易误判成后端挂了。**判据：看 Go 日志里是不是 401**（而不是 500/超时）。
   所以登录步骤必须和后续操作写在**同一次调用**里。
